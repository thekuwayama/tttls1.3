...
RSpec.describe EchAuth do
  ...
  let(:retry_config) do
    unsigned = EchAuth::ECHAuth.new(not_after, 0, spki, algorithm, '')
  ...
  context 'retry_config, whose ech_auth is signed by a trusted key' do
    it 'should be authenticated' do
      expect(EchAuth.authenticated?(retry_config, trusted_keys, Time.now))
        .to be true
    end
    ...
    it 'should NOT be authenticated, if not_after has passed' do
      ...
      expect(EchAuth.authenticated?(retry_config, trusted_keys, now))
        .to be false
      expect(EchAuth.authenticated?(retry_config, trusted_keys, now + 1))
        .to be false
    end
  end
  ...
  context 'retry_config, whose signature covers other bytes' do
    it 'should NOT be authenticated' do
      expect(EchAuth.authenticated?(unauthentic, trusted_keys, Time.now))
        .to be false
    end
  end
  ...
  context 'retry_config, which has NO ech_auth' do
    it 'should NOT be authenticated' do
      expect(EchAuth.authenticated?(echconfig(nil), trusted_keys, Time.now))
        .to be false
    end
  end
  ...
  context 'retry_configs' do
    it 'should be filtered by authentication' do
      expect(
        EchAuth.authenticate([unauthentic, retry_config, echconfig(nil)],
                             trusted_keys,
                             Time.now)
      ).to eq [retry_config]
    end
  ...
  context 'ECHConfig, which has ech_authinfo' do
    it 'should record trusted_keys' do
      ...
      expect(EchAuth.trusted_keys(ech_config)).to eq trusted_keys
